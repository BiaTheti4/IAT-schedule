  ...
  <div>
    <table class="table">
      ...
      <tr v-for="cabinet in cabinets" :key="cabinet">
        ...
        <td>
          <label class="switch">
            <input type="checkbox" @change="updateStatusCabinet(cabinet.id,cabinet.status,cabinet.name)"
                   v-model="cabinet.status">
            <span class="slider"></span>
          </label>
          {{ checkStatus(cabinet.status) }}
        </td>
      </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    ...
    checkStatus(status) {
      return (status == 1) ? 'Доступен' : 'Недоступен'
    }
  },
